...
`ruleSteps` steps; the answer is the final row encoded as a natural number
(least-significant bit = cell 0). Deliberately list-based and index-heavy;
submissions are expected to evolve the automaton efficiently (e.g. on a
bit-packed representation) and prove the result against this spec.
-/
